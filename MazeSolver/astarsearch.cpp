...
void AStarSearch::solve(Maze &maze) {
    ...
    priority_queue<pair<Cell*, int>, vector<pair<Cell*, int>>, Comparator> pq;
    unordered_map<Cell*, Cell*> path;

    Cell* startCell = maze.getStartCell();
    Cell* finishCell = maze.getFinishCell();
    pq.push(pair<Cell*, int>(startCell, manhattanDistance(startCell, finishCell)));

    while (!pq.empty()) {
        Cell* current = pq.top().first;
        pq.pop();
        ...
        if(!walls[Wall::NORTH] && !maze.getMaze()[current->getY() - 1][current->getX()]->wasVisited()) {
            pq.push(pair<Cell*, int>(maze.getMaze()[current->getY() - 1][current->getX()], manhattanDistance(maze.getMaze()[current->getY() - 1][current->getX()], finishCell)));
        ...
        if(!walls[Wall::EAST] && !maze.getMaze()[current->getY()][current->getX() + 1]->wasVisited()) {
            pq.push(pair<Cell*, int>(maze.getMaze()[current->getY()][current->getX() + 1], manhattanDistance(maze.getMaze()[current->getY()][current->getX() + 1], finishCell)));
